"""
The :py:mod:`first_task` application ships with some custom system
checks which ensure that required settings are present. These system checks are registered by
the :py:class:`~first_task.apps.Config` class's
:py:meth:`~first_task.apps.AssetsConfig.ready` method.
    ...
    for idx, name in enumerate(REQUIRED_SETTINGS):
        ...
        if value is None or value == '':
            errors.append(Error(
                ...
                id='first_task.E{:03d}'.format(idx + 1),
